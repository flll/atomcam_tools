...
// 実機の映像は 16:9(1920x1080) なので、モックも同じアスペクト比にする
// (4:3 だと実機で起きるレイアウト崩れを開発中に再現できない)。
export function makeFrameSvg(pan: number, tilt: number): string {
  ...
  const cy = 180 + Math.sin(t * 1.1) * 90;
  ...
  <text x="16" y="348" font-family="monospace" font-size="16" fill="#e2e8f0">${ts}</text>
