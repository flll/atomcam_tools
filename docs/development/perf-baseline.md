...
## Phase 2 第2弾(2026-07-05・4項目採用+2項目データで見送り)

| 項目 | 判定 | 実測根拠 |
|------|------|----------|
| #11 webhook 機能復活(stdout→FIFO 復元+respawn ループ) | **採用**(4302bac) | awk storm 根治済みが前提。検証: iCamera fd1→/atom/run/atomapp、awk は voluntary ctxt 主体(3536/24)で健全、smoke 全 pass。timelapse_hook/motor reset/WEBHOOK 通知が復帰 |
| #7 MDNS=off ゲート(S30dbus/S50avahi overlay 化) | **採用**(8539b1c+fc43a17) | off 時 avahi+dbus を ps から排除(RAM 約8MB)。既定 on。実機は MDNS=off 運用(.local 不要・IP 直指定) |
| #9 S80tailscale enable 事前判定 | **採用**(76afdd5) | 無効時 1.8s→0.03s(bash 起動回避) |
| #4 SD 空き床ガード | **採用**(f39fdbf) | SD 95% の主因は record/ 12.8GB(PERIODICREC_SDCARD_REMOVE=off の意図的設定)。保持設定は尊重し、空き1GB未満で最古日から1.5GB回復まで削除する安全網のみ追加 |
| #10 assis/hl_client 無効化 | **見送り** | 実測 RSS は assis 2.0MB / hl_client 0.9MB(VSZ 96MB は虚像)・CPU 0%。効果3MBにクラウド連携喪失リスクは見合わない |
| #5 vendor rtsp x16 | **見送り**(再確認) | 健全ブートでは rtsp x1+live555 x1 のみを再確認 |

### ブート時間の現状整理(plain boot)

- 制御可能部分は最適化完了: S15swap 0.5s / S55sshd 0.3s / S30・S50・S80 ほぼ0 / S61 6.7〜13s(iCamera 本体)
- **残る変動要因は S41network(WiFi 接続)で 5〜31s のブレ**(2026-07-05 実測 31.1s、環境起因)。
  boot_total は 30〜46s のレンジ。さらに詰めるなら DHCP 静的リース化や wpa 再試行チューニングが候補(未着手)
